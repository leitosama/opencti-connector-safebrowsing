...
class SafeBrowsingConnector:
    ...
    def safe_browsing(self, observable):

        self.helper.log_info(
            f"Checking domain {observable['value']} against Safe Browsing"
        )

        """Checks a domain against the Safe Browsing API."""

        API_KEY = os.environ.get("SAFE_BROWSING_API_KEY", "")
        API_URL = os.environ.get("SAFE_BROWSING_API_URL", "")
        domain = observable["value"]
        stix_objects = []
        url = f"{API_URL}/v4/threatMatches:find?key={API_KEY}"
        payload = {
            "client": {
    ...
    def process_message(self, data: Dict):
        ...
        if observable["entity_type"] == "Domain-Name":
            self.helper.log_info(
                f"Checking domain {observable} against Safe Browsing"
            )
            return self.safe_browsing(observable)
        elif observable["entity_type"] == "Url":
            return self.safe_browsing(observable)
        elif observable["entity_type"] == "Hostname":
            return self.safe_browsing(observable)
