# OpenCTI Safe Browsing Connector

| Status | Date | Comment |
|--------|------|---------|
| Community | -    | -       |

The Safe Browsing connector enriches Domain-Name, URL, and Hostname observables by checking them against Google or Yandex Safe Browsing threat lists, identifying malware, phishing, and unwanted software threats.

## Table of Contents

- [OpenCTI Safe Browsing Connector](#opencti-safe-browsing-connector)
...
Safe Browsing is a service that lets client applications check URLs against Google's OR Yandex constantly updated lists of unsafe web resources. These include social engineering sites (phishing and deceptive sites), sites that host malware or unwanted software, and sites that host potentially harmful applications.

This connector integrates Safe Browsing with OpenCTI to:
...
- Safe Browsing API key (from Google Cloud Console OR Yandex Dev Portal)
...
| Connector Name       | name                 | `CONNECTOR_NAME`                 | Safe Browsing       | No        | Name of the connector.                                                      |
...
| Parameter        | config.yml | Docker environment variable      | Mandatory | Description                                    |
|------------------|------------|----------------------------------|-----------|------------------------------------------------|
| API Key          |            | `SAFE_BROWSING_API_KEY`   | Yes       | Safe Browsing API key from Cloud Console.|
| API URL          |            | `SAFE_BROWSING_API_URL`   | Yes       | Safe Browsing API key from Cloud Console.|
...
```bash
docker build -t leitosama/opencti-connector-safebrowsing:6.9.10 .
```

Configure the connector in `docker-compose.yml`:

```yaml
  connector-safebrowsing:
    image: leitosama/opencti-connector-safebrowsing:6.9.10
    environment:
      - OPENCTI_URL=http://localhost
      - OPENCTI_TOKEN=ChangeMe
      - CONNECTOR_ID=ChangeMe_UUID4
      - CONNECTOR_NAME=Safe Browsing
      - CONNECTOR_SCOPE=Url,Domain-Name,Hostname
      - CONNECTOR_LOG_LEVEL=info
      - CONNECTOR_AUTO=true
      - CONNECTOR_UPDATE_EXISTING_DATA=true
      - SAFE_BROWSING_API_KEY=ChangeMe
      - SAFE_BROWSING_API_URL=https://sba.yandex.net/v4
    restart: always
```
...
The connector checks observables against Safe Browsing threat lists.

**Observations → Observables**

Select a Domain-Name, URL, or Hostname observable, then click the enrichment button and choose Safe Browsing.

## Behavior

The connector queries the Safe Browsing API to check if URLs/domains are flagged as threats.
...
- **Rate Limits**: Subject to Google OR Yandex API rate limits
